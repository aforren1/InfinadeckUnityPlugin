...
/**
 * ------------------------------------------------------------
 * Core Script of InfinaDATA (Infinadeck.Data)
 ...
 *
 * Ricardo Amores Hernández's "Ini Parser" DLL can be found under the package's Runtime/Plugins folder,
...
namespace Infinadeck
{
    public class Data : MonoBehaviour
    {
        ...
        [ReadOnlyInEditor] public string lastSync_ymd;
        [ReadOnlyInEditor] public string lastSync_hms;
        ...
        IEnumerator CheckGamePreferences()
        {
            while (true)
            {
                ...
                foreach (KeyValuePair<string, DataEntry> pref in all)
                {
                    ...
                    {
                        Debug.Log("[Infinadeck.Data] " + pref.Value.EntryName + " changed; saving to " + path);
                        SaveSettings();
                        break;
                    }
                }
